...
% hp = scatter(slons(nnan),slats(nnan),140,nanmean(all_dtstar(nnan,:),2),'filled','MarkerEdgeColor','k');
hp = scatter(slons(nnan),slats(nnan),scale*sqrt(sum(~isnan(all_dtstar(nnan,:)),2)),sta_terms(nnan),'filled','MarkerEdgeColor','k');
...
% save
if ifsave
save2pdf(31,sprintf('dtstar_%s_%s_sta_average',phase,component),'figs');
end
