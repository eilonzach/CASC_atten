...
par.damp            = 0.1; %3 % scaling for whole damp mat: more damping parms found in dampmat.m
	par.damp_evt    = 0.01e7; %1 % multiple of rest of model damping to use for evt terms
...
par.smooth          = 20; %3 %  scaling for whole smth mat: good result with 1 - 
    par.smth_zvh    = 0.3; %0.3 %  Ratio of vertical to horizontal smoothing
...
%% synth model parms
    ...
    % custom structure
    sym.acx = [-815	 -772  -687  -637  -566  -630  -151.5]           ;% vector of x-coords for anomaly centres
    sym.acy = [ 103	  262   400   565  -153  -310    24  ]           ;% vector of y-coords for anomaly centres
    sym.acz = [  50	   50    50    50    50    50   180  ]           ;% vector of z-coords for anomaly centres
    
    sym.awx = [  80	   60    80    80    80    60   100]           ;% vector of x-widths for anomalies (km)
    sym.awy = [ 120	  140   120   150   120   120  1000]           ;% vector of y-widths for anomalies (km)
    sym.awz = [  90	   90    90    90    90    90   300]           ;% vector of z-widths for anomalies (km)

    sym.adval=[  5	   5    5    5    5    5    -1]           ;% vector of pertubations of anomalies (percent)
    
    % checker structure
    sym.dval = 5;                        % in percent!
        
    sym.noise = 0.1;                     % 0.05 % std. of gaussian perturbations to data values (s)
    sym.estatic_sd = 0.0;               % 0.5 % std. of gaussian perturbations input event terms (s)
    sym.sstatic_sd = 0.0;               % 0.5 % std. of gaussian perturbations input station terms (s)
