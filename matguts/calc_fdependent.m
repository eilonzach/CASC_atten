function [ delta_tstar_pref,delta_T_pref,A0_pref,alpha_pref,alpha_VR,alpha_Ew ] ...
...
%  = calc_fdependent( Amat,phimat,fmids,test_alphas,wtmat,[amp2phiwt=5],[opt=1],[titstr=''],ifplot )
...
if opt==1
 %% all in one method
    [ ~,~,~,~,~,dtstars_a,dTs_a,A0s_a ] ...
        = invert_allin1_Aphis_4_STA_dtdtstar_alpha(Amat,phimat,fmids,test_alphas,wtmat,amp2phiwt,[],ifplot );
    methstr = 'all in one';
elseif opt == 2
   %% one-by-one method
    [ ~,~,~,~,~,dtstars_a,dTs_a,A0s_a ] ...
...
for ia = 1:length(test_alphas)
    ...
    veaw = nanvariance((log(Amat_pred(:)) - log(Amat(:))).*wtmat(:)); % variance of the error in Amp
    vaw = nanvariance(log(Amat(:)).*wtmat(:));                        % variance of the Amp
    VRwa(ia) = 1 - veaw/vaw;                                          % variance reduction in Amp
    
    vepw = nanvariance((phimat_pred(:) - phimat(:)).*wtmat(:));       % variance of the error in phi
    vpw = nanvariance(phimat(:).*wtmat(:));                           % variance of the phi
