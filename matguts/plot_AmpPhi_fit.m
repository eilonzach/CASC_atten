function [misfit_amp,misfit_phi,E_a,E_p] = plot_AmpPhi_fit( Amp,Phi,fmids,wts,dtstar,dT,A0, alp,fign_or_ax,w0 )
% [misfit_amp,misfit_phi] = plot_AmpPhi_fit( Amp,Phi,fmids,[wts=1],dtstar,dT,A0, [alpha=0], [fign_or_ax=33], [w0=2pi] )
% 
...
if nargin < 9 || isempty(fign_or_ax)
    fign_or_ax = 33;
end
...
if all(~ishandle(fign_or_ax)) || length(fign_or_ax)==1
    figure(fign_or_ax), clf, set(gcf,'pos',[600 600 600,800])
    ax1 = subplot(2,1,1); hold on
    ax2 = subplot(2,1,2); hold on
else
    ax1 = fign_or_ax(1); hold on
    ax2 = fign_or_ax(2); hold on
end

% ================ PLOT THE AMPLITUDE SPECTRA ================

scatter(ax1,fmids(wts~=0),log(Amp(wts~=0)),150*wts(wts~=0),'ok','MarkerFaceColor','r','linewidth',1.5)
...
set(ax1,'fontsize',15,'Xscale','linear','linewidth',2,'box','on','xlim',[0 max(fmids)])
...
scatter(ax2,fmids(wts~=0),Phi(wts~=0),150*wts(wts~=0),'ok','MarkerFaceColor','r','linewidth',1.5)
...
set(ax2,'fontsize',15,'Xscale','linear','linewidth',2,'box','on','xlim',[0 max(fmids)])

ax = axis; xl = ax(1) + 0.05*(ax(2)-ax(1)); yl = ax(4) - 0.1*(ax(4)-ax(3));
% text(ax2,xl,yl,['Weighted misfit = ',num2str(misfit_phi)],'interpreter','latex','fontsize',20,'horizontalalignment','left','verticalalignment','bottom')

return
% ================ SAVE FIGURE ================
ofile = sprintf('synth_comb_Q1-%.0f_Q2-%.0f',Q0_1,Q0_2);
save2pdf(gcf,ofile,'figs');
