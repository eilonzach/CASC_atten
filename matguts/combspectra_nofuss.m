function [delta_tstar,delta_T,std_dtstar,pairwise,fmids] = combspectra_nofuss(dat,samprate,parms,ifplot)
...
%     [fltdat(iw).bb,fltdat(iw).aa]=cheby1(parms.comb.npol,0.5, [flos(iw), fhis(iw)].*dt.*2);
    ...
    fltdat(iw).bb=sos; 
    fltdat(iw).aa=g;
    fltdat(iw).fmid = fmids(iw);
    fltdat(iw).flo = flos(iw);
    fltdat(iw).fhi = fhis(iw);
end
...
for is2 = is1+1:Nstas
    ...
    [ As,phis,wts ] = run_comb( dat(:,is1),dat(:,is2),fltdat,wdo1,wdo2,jbds,dt,pretime,maxphi,corc_skip,fmin,ifplot );
...
%         [ dtstar_i,dT_i,A0_i,misfit_i,E_i ] = invert_Aphi_4_dtdtstar( As(doinds),phis(doinds),fmids(doinds),fnq, wts(doinds),amp2phiwt);
...
pairwise = struct('dtstar',dtstar_pairwise,'dT',dT_pairwise,'misfit_normed',misfitnormed_pairwise,...
                  'As',As_pairwise,'phis',phis_pairwise,'wts',wts_pairwise);
